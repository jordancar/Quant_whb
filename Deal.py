    ...
    def __init__(self,state_dt):
        ...
        try:
            ...
            for i in range(len(done_set2)):
                sql = "select * from stock_all_plus a where a.stock_code = '%s' and a.state_dt = '%s'"%(done_set2[i][0],state_dt)
                cursor.execute(sql)
                ...
                try:
                    ...
                    self.cur_money_lock += float(done_set2[i][1]) * float(done_set2[i][2])
            ...
            #db.rollback()
            print 'Class of Deal,error occured...'
            print(excp)
            x=raw_input("deal error...")
